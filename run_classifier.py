...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf1.logging.info("guid: %s" % (example.guid))
    tf1.logging.info("tokens: %s" % " ".join(
        [tokenization.printable_text(x) for x in tokens]))
    tf1.logging.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
    tf1.logging.info("input_mask: %s" % " ".join([str(x) for x in input_mask]))
    tf1.logging.info("segment_ids: %s" % " ".join([str(x) for x in segment_ids]))
    tf1.logging.info("label: %s (id = %d)" % (example.label, label_id))
...
def file_based_convert_examples_to_features(
  ...
  writer = tf1.python_io.TFRecordWriter(output_file)

  for (ex_index, example) in enumerate(examples):
    if ex_index % 10000 == 0:
      tf1.logging.info("Writing example %d of %d" % (ex_index, len(examples)))

    feature = convert_single_example(ex_index, example, label_list,
                                     max_seq_length, tokenizer)

    def create_int_feature(values):
      f = tf1.train.Feature(int64_list=tf1.train.Int64List(value=list(values)))
      return f
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "label_ids": tf1.FixedLenFeature([], tf1.int64),
      "is_real_example": tf1.FixedLenFeature([], tf1.int64),
  }
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if "is_real_example" in features:
      is_real_example = tf1.cast(features["is_real_example"], dtype=tf1.float32)
    else:
      is_real_example = tf1.ones(tf1.shape(label_ids), dtype=tf1.float32)
    ...
    if mode == tf1.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        loss = tf1.metrics.mean(values=per_example_loss, weights=is_real_example)
        return {
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  def input_fn(params):
    ...
    # not use Dataset.from_generator() because that uses tf1.py_func which is
    # not TPU compatible. The right way to load data is with TFRecordReader.
    d = tf1.data.Dataset.from_tensor_slices({
        "input_ids":
            tf1.constant(
        ...
        "segment_ids":
            tf1.constant(
                ...
                dtype=tf1.int32),
        "label_ids":
            tf1.constant(all_label_ids, shape=[num_examples], dtype=tf1.int32),
    })
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    if FLAGS.use_tpu:
      ...
      # later on. These do NOT count towards the metric (all tf1.metrics
    ...
    tf1.logging.info("  Num examples = %d (%d actual, %d padding)",
